...
!**********************************************************************C
!
      SUBROUTINE SUBCHAN(QCHANUT,QCHANVT,IACTIVE,RLAMN,RLAMO,DELT,IACTALL)
!
...
!            IF(H1P(LCHNU).GT.HWET)THEN
            IF(H1P(LCHNU).GT.HDRY)THEN
              HCHNCOR=HWET
              IACTIVE(NMD)=1
              IACTALL=IACTALL+1
!              IF(ISCDRY(LCHNU).EQ.1) IACTIVE=1
            ENDIF
            ENDIF
...
!                CCCCHV(NMD)=DELT*HCHANT*WCHAN/RLCHN
...
!         Y-DIRECTION CHANNEL
          IF(MDCHTYP(NMD).EQ.2)THEN
            ...
            IF(SRFCHAN.GT.SRFHOST)THEN
!            IF(H1P(LCHNV).GT.HWET)THEN
            IF(H1P(LCHNV).GT.HDRY)THEN
!              HCHNMX=H1P(LCHNV)+BELV(LCHNV)-BELV(LHOST)
              HCHNMX=-H1P(LCHNV)
              IHCHMX=1
              IACTIVE(NMD)=1
              IACTALL=IACTALL+1
...
!                CCCCHV(NMD)=DELT*HCHANT*WCHAN/RLCHN
        ...
        ENDDO
!
!        WRITE(8,1949)N,IACTALL       !hnr 7/27/2009
!
!        DO NMD=1,MDCHH
!	    WRITE(8,1948)NMD,CCCCHH(NMD),CCCCHU(NMD),CCCCHV(NMD)
!        ENDDO
!
      ENDIF
!
