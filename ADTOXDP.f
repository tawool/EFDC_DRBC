...
!TAW               CUPP(L,K)=DMIN1(QWTRBED_DP(L,K),0.) 
!TAW     &          -((DIFTOX_DP(NT)/(HBED_DP(L,K)+HBED_DP(L,K+1)))+
!TAW     &          PARTDIF_DP(L,K))*(PORBED_DP(L,K)+PORBED_DP(L,K+1))
C###############################################################################
              ENDDO
!TAW              CUPP(L,KBT(L))=AMIN1(QWTRBED_DP(L,KBT(L)),0.)
!TAW     &          -DIFBWFAC*DIFTOXBW(L)*PORBED_DP(L,KBT(L))
C
              DO K=2,KBT(L)
C###############################################################################
C HQI Change to implement particle mixing as a rate in units of length/time
...
!TAW                ALOW(L,K)=-AMAX1(QWTRBED_DP(L,K-1),0.)
!TAW     &          -((DIFTOX_DP(NT)/(HBED_DP(L,K-1)+HBED_DP(L,K)))+
!TAW     &          PARTDIF_DP(L,K-1))*(PORBED_DP(L,K-1)+PORBED_DP(L,K))
C###############################################################################
              ENDDO
!TAW              ALOW(L,KBTP1)=-AMAX1(QWTRBED_DP(L,KBT(L)),0.)
!TAW     &                 -DIFBWFAC*DIFTOXBW(L)*PORBED_DP(L,KBT(L))
C
              DO K=1,KBT(L)
...
!TAW              BMNN(L,1)=BMNN(L,1)+AMAX1(QWTRBED_DP(L,1),0.)
!TAW     &        +((DIFTOX_DP(NT)/(HBED_DP(L,2)+HBED_DP(L,1)))+
!TAW     &        PARTDIF_DP(L,1))*(PORBED_DP(L,2)+PORBED_DP(L,1))
C###############################################################################
              DO K=2,KBTM1
C###############################################################################
C HQI Change to implement particle mixing as a rate in units of length/time
...
c     &                  +PORBED_DP(L,K))/
c     &                  (HBED_DP(L,K-1)+HBED_DP(L,K))
!TAW                BMNN(L,K)=BMNN(L,K)+AMAX1(QWTRBED_DP(L,K),0.)
!TAW     &         +((DIFTOX_DP(NT)/(HBED_DP(L,K+1)+HBED_DP(L,K)))+
!TAW     &         PARTDIF_DP(L,K))*(PORBED_DP(L,K+1)+PORBED_DP(L,K))
!TAW     &                         -AMIN1(QWTRBED_DP(L,K-1),0.)
!TAW     &         +((DIFTOX_DP(NT)/(HBED_DP(L,K-1)+HBED_DP(L,K)))+
!TAW     &         PARTDIF_DP(L,K-1))*(PORBED_DP(L,K-1)+PORBED_DP(L,K))
C###############################################################################
              ENDDO
...
C###############################################################################
              K=KBTP1
!TAW              BMNN(L,K)=BMNN(L,K)-AMIN1(QWTRBED_DP(L,K-1),0.)
!TAW     &          +DIFBWFAC*DIFTOXBW(L)*PORBED_DP(L,KBT(L))
C
              DO K=1,KBT(L)
              ...
              ENDDO
!TAW              RRHS(L,1)=RRHS(L,1)+AMAX1(QWTRBED_DP(L,0),0.)
!TAW     &                  *CONGW_DP(L,NT+4)
