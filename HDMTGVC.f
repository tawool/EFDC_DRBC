...
C
C 12/18/2020        DRBC LZ - ADD CALCULATION OF TURBULENCE DISSIPATION RATE "DISPRATE"
C 01/14/2021        DRBC LZ - PERFORM LOG-LINEAR EXTRAPOLATION FOR "DISPRATE"
C 03/24/2022        DRBC LZ - MODIFY "DISPRATE" FOR 1-D SCENARIO
C
C----------------------------------------------------------------------C
C
C**********************************************************************C
...
C----------------------------------------------------------------------C
C
C MODIFIED FOR GVC - JMH 08/03/04
C
      DO L=2,LA
	KBU=KGVCU(L)
	KBV=KGVCV(L)
      ...
      UV(L)=0.25*(HP(LS)*(U(LSE,KBV)+U(LS,KBV))
     &         +HP(L)*(U(L+1,KBV)+U(L,KBV)))*HVI(L)
      U1V(L)=0.25*(H1P(LS)*(U1(LSE,KBV)+U1(LS,KBV))
     &          +H1P(L)*(U1(L+1,KBV)+U1(L,KBV)))*H1VI(L)
      VU(L)=0.25*(HP(L-1)*(V(LNW,KBU)+V(L-1,KBU))
     &         +HP(L)*(V(LN,KBU)+V(L,KBU)))*HUI(L)
      V1U(L)=0.25*(H1P(L-1)*(V1(LNW,KBU)+V1(L-1,KBU))
     &          +H1P(L)*(V1(LN,KBU)+V1(L,KBU)))*H1UI(L)
      ENDDO
C
C**********************************************************************C
C
...
C
     ...
     &        +U1(L,KBU)*U1(L,KBU)))*U1(L,KBU)
        TBY1(L)=(AVCON1*H1VI(L)+STBY(L)*SQRT(U1V(L)*U1V(L)
     &        +V1(L,KBV)*V1(L,KBV)))*V1(L,KBV)
        TSX1(L)=TSX(L)
        TSY1(L)=TSY(L)
      ENDDO
C
...
C
     ...
     &       +U(L,KBU)*U(L,KBU)))*U(L,KBU)
        TBY(L)=(AVCON1*HVI(L)+STBY(L)*SQRT(UV(L)*UV(L)
     &       +V(L,KBV)*V(L,KBV)))*V(L,KBV)
      ENDDO
C
C      IF(ISVEG.GE.1.AND.KC.GT.1)
C      DO ND=1,NDM
...
C
      DO L=2,LA
	 KBPM=KGVCP(L)-1
       QQ1(L,KBPM)=0.5*CTURB2*SQRT((TVAR3E(L)+TBX1(L))**2
     &                        +(TVAR3N(L)+TBY1(L))**2)
...
C
      DO L=2,LA
	 KBPM=KGVCP(L)-1
       QQ(L,KBPM)=0.5*CTURB2*SQRT((TVAR3E(L)+TBX(L))**2
     &                        +(TVAR3N(L)+TBY(L))**2)
...
C
cjh11/15/06      DO L=2,LA
cjh11/15/06        UHDY2E(L)=0.
cjh11/15/06        VHDX2E(L)=0.
cjh11/15/06        HWQ(L)=0.25*(H2P(L)+2.*H1P(L)+HP(L))
cjh11/15/06      ENDDO
cjh11/15/06C
cjh11/15/06      DO K=1,KC
cjh11/15/06       DO L=2,LA
cjh11/15/06        UHDYWQ(L,K)=0.5*(UHDY1(L,K)+UHDY2(L,K))
cjh11/15/06        VHDXWQ(L,K)=0.5*(VHDX1(L,K)+VHDX2(L,K))
cjh11/15/06        UHDY2E(L)=UHDY2E(L)+GVCSCLU(L)*UHDYWQ(L,K)*DZC(K)
cjh11/15/06        VHDX2E(L)=VHDX2E(L)+GVCSCLV(L)*VHDXWQ(L,K)*DZC(K)
cjh11/15/06        UWQ(L,K)=0.5*(U1(L,K)+U2(L,K))
cjh11/15/06        VWQ(L,K)=0.5*(V1(L,K)+V2(L,K))
cjh11/15/06       ENDDO
cjh11/15/06      ENDDO
cjh11/15/06C
cjh11/15/06      DO L=2,LA
cjh11/15/06       TVAR3E(L)=GVCSCLP(L)*UHDY2E(L+1)
cjh11/15/06       TVAR3N(L)=GVCSCLP(L)*VHDX2E(LNC(L))
cjh11/15/06      ENDDO
cjh11/15/06C
cjh11/15/06      DO K=1,KC
cjh11/15/06      DO L=2,LA
cjh11/15/06        TVAR2E(L,K)=GVCSCLU(L+1   )*UHDYWQ(L+1   ,K)
cjh11/15/06        TVAR2N(L,K)=GVCSCLV(LNC(L))*VHDXWQ(LNC(L),K)
cjh11/15/06      ENDDO
cjh11/15/06      ENDDO
cjh11/15/06C
cjh11/15/06      DO K=1,KS
cjh11/15/06        DO L=2,LA
cjh11/15/06          WWQ(L,K)=SWB3D(L,K)*(WWQ(L,K-1)
cjh11/15/06     &       -DZC(K)*(TVAR2E(L,K)-GVCSCLU(L)*UHDYWQ(L,K)
cjh11/15/06     &               -TVAR3E(L)+GVCSCLP(L)*UHDY2E(L)
cjh11/15/06     &               +TVAR2N(L,K)-GVCSCLV(L)*VHDXWQ(L,K)
cjh11/15/06     &               -TVAR3N(L)+GVCSCLP(L)*VHDX2E(L))*DXYIP(L))
cjh11/15/06     &    +SWB3D(L,K)*( QSUM(L,K)-DZC(K)*GVCSCLP(L)*QSUME(L) )*DXYIP(L)
cjh11/15/06        ENDDO
cjh11/15/06      ENDDO
cjh11/15/06C
cjh11/15/06      IF(ISDIVEX.EQ.3)THEN
cjh11/15/06	  OPEN(1,FILE='DIV3GVC.OUT')
cjh11/15/06	  CLOSE(1,STATUS='DELETE')
cjh11/15/06	  OPEN(1,FILE='DIV3GVC.OUT')
cjh11/15/06        K=KC
cjh11/15/06        DO L=2,LA
cjh11/15/06          TVAR3S(L)=(WWQ(L,K-1)
cjh11/15/06     &       -DZC(K)*(TVAR2E(L,K)-GVCSCLU(L)*UHDYWQ(L,K)
cjh11/15/06     &               -TVAR3E(L)+GVCSCLP(L)*UHDY2E(L)
cjh11/15/06     &               +TVAR2N(L,K)-GVCSCLV(L)*VHDXWQ(L,K)
cjh11/15/06     &               -TVAR3N(L)+GVCSCLP(L)*VHDX2E(L))*DXYIP(L))
cjh11/15/06     &    +( QSUM(L,K)-DZC(K)*GVCSCLP(L)*QSUME(L) )*DXYIP(L)
cjh11/15/06        ENDDO
cjh11/15/06	  DO L=2,LA
cjh11/15/06	    WRITE(1,1492)IL(L),JL(L),TVAR3S(L)
cjh11/15/06	  ENDDO
cjh11/15/06	  CLOSE(1)
cjh11/15/06	ENDIF
cjh11/15/06c
cjh11/15/06      DO L=2,LA
cjh11/15/06       TVAR3E(L)=UHDY2E(L+1)
cjh11/15/06       TVAR3N(L)=VHDX2E(LNC(L))
cjh11/15/06      ENDDO
cjh11/15/06C
cjh11/15/06      DO L=2,LA
cjh11/15/06        HPPTMP=H2WQ(L)+DT2*DXYIP(L)*(QSUME(L)
cjh11/15/06     &      -(TVAR3E(L)-UHDY2E(L)+TVAR3N(L)-VHDX2E(L)))
cjh11/15/06        HWQ(L)=SPB(L)*HPPTMP+(1.-SPB(L))*HWQ(L)
cjh11/15/06      ENDDO
C
cjh11/15/06 - temporarily modified to use 3t hydro transport
C
       DO L=2,LA
        HWQ(L)=HP(L)
        WWQ(L,0)=0.
...
C
	  ...
	  OPEN(1,FILE='DIV3GVC.OUT')
        ...
        DO L=2,LA
          TVAR3S(L)=(WWQ(L,K-1)
     &       -DZC(K)*(TVAR2E(L,K)-GVCSCLU(L)*UHDYWQ(L,K)
     &               -TVAR3E(L)+GVCSCLP(L)*UHDY2E(L)
     &               +TVAR2N(L,K)-GVCSCLV(L)*VHDXWQ(L,K)
     &               -TVAR3N(L)+GVCSCLP(L)*VHDX2E(L))*DXYIP(L))
     &    +( QSUM(L,K)-DZC(K)*GVCSCLP(L)*QSUME(L) )*DXYIP(L)
        ENDDO
	  DO L=2,LA
...
C      IF(N.LE.6)THEN
C      DO L=2,LA
C	  WRITE(8,881)N,IL(L),JL(L),KGVCU(L),TBX(L),STBX(L),
C     &             U(L,KGVCU(L)),VU(L)
C      ENDDO
C      ENDIF
C  881 FORMAT('TBX GVC ',4I5,10E14.6)
CGVCDIAG
c
C
C      IF(ISVEG.GE.1.AND.KC.GT.1)
...
C**********************************************************************C
C
C **  CALCULATE TURBULENCE DISSIPATION RATE, 12/16/2020, 3/24/2022, DRBC LZ
C
      DO L=2, LA
