...
!             QQI(L)=1./(QQMIN+QQ(L,K))
	    ...
	    IF(LMASKDRY(L))THEN
	    IF(LGVCP(L,K))THEN
            RIQ=-GP*GVCSCLPI(L)*HP(L)*DML(L,K)*DML(L,K)*DZIG(K)*(B(L,K+1)-B(L,K))*QQI(L)
            RIQ=MAX(RIQ,RIQMIN)
...
!
	      ...
	      IF(STUFF.LT.0.0)WRITE(6,6999)IL(L),JL(L),K,AV(L,K),AVTMP,GVCSCLPI(L),HPI(L),HP(L)
...
!      IF(N.LE.2)THEN
!      DO L=2,LA
!	  WRITE(8,881)N,IL(L),JL(L),(AV(L,K),K=1,KS)
!      ENDDO
!      ENDIF
!
!  881 FORMAT('AV GVC ',3I5,10E14.6)
!GVCDIAG
!----------------------------------------------------------------------C
!
...
!
      DO L=2,LA
      IF(KGVCP(L).LT.KC)AQ(L,KGVCP(L))=0.314*DML(L,KGVCP(L))**SQRT(QQ(L,KGVCP(L)))+RAVBTMP*AVO*HPI(L)
...
!
      DO L=2,LA
      IF(KGVCP(L).LT.KC)AQ(L,KGVCP(L))=0.5*( 0.314*DML(L,KGVCP(L))**SQRT(QQ(L,KGVCP(L)))+RAVBTMP*AVO*HPI(L) ) +0.5*AQ(L,KGVCP(L))
