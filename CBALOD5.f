...
C
      ...
      UMOENDO=UMOENDO+SPB(L)*0.5*DXYP(L)*HP(L)*(DYIU(L)*HUI(L)*UHDYE(L)
     &                                 +DYIU(L+1)*HUI(L+1)*UHDYE(L+1))
      VMOENDO=VMOENDO+SPB(L)*0.5*DXYP(L)*HP(L)*(DXIV(L)*HVI(L)*VHDXE(L)
...
C     UUEENDO=UUEENDO+SPB(L)*0.25*(DXYU(L)*HU(L)*U(L,K)*U(L,K)
C    &      +DXYU(L+1)*HU(L+1)*U(L+1,K)*U(L+1,K))*DZC(K)
C     VVEENDO=VVEENDO+SPB(L)*0.25*(DXYV(L)*HV(L)*V(L,K)*V(L,K)
