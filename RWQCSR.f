...
!**********************************************************************C
!
!     READ AND UPDATE WATER COLUMN WATER QUALITY VARIABLE TIME SERIES
!
!**********************************************************************C
!
      INCLUDE 'EFDC.PAR'
...
!
        DO NS=1,NWQCSR(NW)
        MWQCTLT(NS,NW)=1
        READ(1,*,IOSTAT=ISO)ISTYP,MWQCSR(NS,NW),TCWQCSR(NS,NW),TAWQCSR(NS,NW),RMULADJ,ADDADJ
        ...
        IF(ISTYP.EQ.1)THEN
         ...
         ELSE
          DO M=1,MWQCSR(NS,NW)
          READ(1,*,IOSTAT=ISO)TWQCSER(M,NS,NW),(WQCSER(M,K,NS,NW), K=1,KC)
          IF(ISO.GT.0) GOTO 900
          TWQCSER(M,NS,NW)=TWQCSER(M,NS,NW)+TAWQCSR(NS,NW)
           DO K=1,KC
...
!**********************************************************************C
!
! **  CONCENTRATION SERIES INTERPOLTATION FOR WATER QUALITY VARIABLES
!
      DO NW=1,NWQV
!
       DO NS=1,NWQCSR(NW)
...
!
       ...
       WTM2=(TIME-TWQCSER(M1,NS,NW))/TDIFF
        DO K=1,KC
         CSERTWQ(K,NS,NW)=WTM1*WQCSER(M1,K,NS,NW)+WTM2*WQCSER(M2,K,NS,NW)
        ENDDO
...
!**********************************************************************C
!
! **  ON FIRST CALL, INITIALIZE OUTFLOW CONCENTRATIONS
!
!----------------------------------------------------------------------C
!
      IF(ITNWQ.EQ.0)THEN
!
      ...
      DO K=1,KC
       DO LL=1,NWQOBS
        ...
        CWQLOS(LL,K,NW)=WTCI(K,1)*WQOBCS(LL,1,NW)+WTCI(K,2)*WQOBCS(LL,2,NW)+CSERTWQ(K,NSID,NW)
        NWQLOS(LL,K,NW)=0
       ENDDO
       DO LL=1,NWQOBW
        NSID=IWQOBW(LL,NW)
        L=LIJ( IWQCBW(LL),JWQCBW(LL) )
        CWQLOW(LL,K,NW)=WTCI(K,1)*WQOBCW(LL,1,NW)+WTCI(K,2)*WQOBCW(LL,2,NW)+CSERTWQ(K,NSID,NW)
        NWQLOW(LL,K,NW)=0
       ENDDO
       DO LL=1,NWQOBE
        NSID=IWQOBE(LL,NW)
        L=LIJ( IWQCBE(LL),JWQCBE(LL) )
        CWQLOE(LL,K,NW)=WTCI(K,1)*WQOBCE(LL,1,NW)+WTCI(K,2)*WQOBCE(LL,2,NW)+CSERTWQ(K,NSID,NW)
        NWQLOE(LL,K,NW)=0
       ENDDO
       DO LL=1,NWQOBN
        NSID=IWQOBN(LL,NW)
        L=LIJ( IWQCBN(LL),JWQCBN(LL) )
        CWQLON(LL,K,NW)=WTCI(K,1)*WQOBCN(LL,1,NW)+WTCI(K,2)*WQOBCN(LL,2,NW)+CSERTWQ(K,NSID,NW)
        NWQLON(LL,K,NW)=0
       ENDDO
      ENDDO
      ENDDO
