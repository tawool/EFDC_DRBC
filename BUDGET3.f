...
!  HNR_GHD, 7/2023   ADDED SALINITY LOAD FOR BUDGET      
!
!----------------------------------------------------------------------C
!
!**********************************************************************C
...
!
      DO NL=1,NLIJ(1)
        ...
        NLSTMP=NSERL(NL,1)
        DO K=1,KC
          SMASSIN=SMASSIN+SERLT(K,NLSTMP,1)    !LOAD IN KG/S OF SALT  
        END DO
      END DO
!
...
!
      ...
      ENDIF
       DO K=1,KC
       SMASSIN=SMASSIN+QCTLT(K,NCTL)*CONT(LU,K)-RQWD*QCTLT(K,NCTL)*CONT(LU,K)
       ENDDO
      ENDDO
!
      DO NWR=1,NQWR
       ...
       NQSTMP=NQWRSERQ(NWR)
       NCSTMP=NQWRSERQ(NWR)
       SMASSIN=SMASSIN+( (QWR(NWR)+QWRSERT(NQSTMP))*CONT(LU,KU) )
       IF(LD.NE.1.OR.LD.NE.LC)THEN
         SMASSIN=SMASSIN-( QWR(NWR)*(CONT(LU,KU)+CQWR(NWR,1))+QSERT(K,NQSTMP)*(CONT(LU,KU)+CQWRSERT(NCSTMP,1)) )
       ENDIF
      ENDDO
!
      ENDIF
...
!
      ...
      ENDIF
       DO K=1,KC
      SEDIN=SEDIN+QCTLT(K,NCTL)*CONT(LU,K)-RQWD*QCTLT(K,NCTL)*CONT(LU,K)
       ENDDO
      ENDDO
!
      DO NWR=1,NQWR
       ...
       NQSTMP=NQWRSERQ(NWR)
       NCSTMP=NQWRSERQ(NWR)
       SEDIN=SEDIN+( (QWR(NWR)+QWRSERT(NQSTMP))*CONT(LU,KU) )
       IF(LD.NE.1.OR.LD.NE.LC)THEN
         SEDIN=SEDIN-( QWR(NWR)*(CONT(LU,KU)+CQWR(NWR,M))+QSERT(K,NQSTMP)*(CONT(LU,KU)+CQWRSERT(NCSTMP,M)) )
       ENDIF
      ENDDO
      ENDDO
!
