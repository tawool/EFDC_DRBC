...
C     (LOOP OVER SURFACE LAYER CELLS)
C
      NFICMS=NFICM/KC
C
      DO M=1,NFICMS
...
C     TRANSPORT IF AVERGING PERIOD NTSMMT IS GE REFERENCE
C     TIME PERIOD NTSPTC
C
      SVPT=0.
...
C
C      DO L=2,LA
C       TVAR3C(L)=TVAR3C(L)+RAINLPF(L)-EVPSLPF(L)
C      ENDDO
C
...
C
      IF(ISDICM.EQ.1)THEN
        ...
        DO L=2,LA
         LM=L-1
         TMPVAL=DXYP(L)*HWQ(L)
C         WRITE(2,294)LM,TVAR3C(L),TMPVAL,L
         WRITE(2,2294)LM,L,HP(L),HWQ(L),HTMP(L)
         WRITE(4,402)LM,L,(QSUMLPF(L,K),K=1,KC)
C         IF(TVAR3C(L).LT.0.0)THEN
C           LP=L+1
C           LN=LNC(L)
C           DO K=1,KC
C             RVAL1= DYU(L)*UHLPF(L,K)*DZC(K)
C             RVAL2=-DYU(LP)*UHLPF(LP,K)*DZC(K)
C             RVAL3= DXV(L)*VHLPF(L,K)*DZC(K)
C             RVAL4=-DXV(LN)*VHLPF(LN,K)*DZC(K)
C             RVAL5= QSUMLPF(L,K)
C             RVAL6=RVAL1+RVAL2+RVAL3+RVAL4+RVAL5
C             WRITE(2,222)L,K,RVAL1,RVAL2,RVAL3,RVAL4,RVAL5,RVAL6
C           ENDDO
C         ENDIF
        ENDDO
      ENDIF
...
CJH       TVAR3N(L)=0.
CJH       DO K=1,KC
CJH        TVAR3E(L)=TVAR3E(L)+DZC(K)*
CJH     &          ( DYU(L)*(UHLPF(L,K)+SVPT*UVPT(L,K))
CJH     &           +DXV(L)*(VHLPF(L,K)+SVPT*VVPT(L,K)) )
CJH        TVAR3N(L)=TVAR3N(L)+DZC(K)*
CJH     &          ( DYU(L+1)*(UHLPF(L+1,K)+SVPT*UVPT(L+1,K))
CJH     &           +DXV(LN )*(VHLPF(LN ,K)+SVPT*VVPT(LN ,K)) )
CJH       ENDDO
CJH      ENDDO
C
C      IF(MDCHH.GE.1)THEN
C        DO NMD=1,MDCHH
C        IF(MDCHTYP(NMD).EQ.1)THEN
C          TVAR3E(LMDCHH(NMD))=TVAR3E(LMDCHH(NMD))
C     &                   +QCHNULP(NMD)
C          TVAR3N(LMDCHU(NMD))=TVAR3N(LMDCHU(NMD))
C     &                   +QCHNULP(NMD)
C        ENDIF
C        IF(MDCHTYP(NMD).EQ.2)THEN
C          TVAR3E(LMDCHH(NMD))=TVAR3E(LMDCHH(NMD))
C     &                   +QCHNVLP(NMD)
C          TVAR3N(LMDCHV(NMD))=TVAR3N(LMDCHV(NMD))
C     &                   +QCHNVLP(NMD)
C        ENDIF
C        IF(MDCHTYP(NMD).EQ.3)THEN
C          TVAR3E(LMDCHH(NMD))=TVAR3E(LMDCHH(NMD))
C     &                   +QCHNULP(NMD)+QCHNVLP(NMD)
C          TVAR3N(LMDCHU(NMD))=TVAR3N(LMDCHU(NMD))
C     &                   +QCHNULP(NMD)+QCHNVLP(NMD)
C        ENDIF
C        ENDDO
...
C       DO L=2,LA
C        LN=LNC(L)
C        TVAR1N(L,K)=DXV(LN)*VHLPF(LN,K)
C        TVAR1S(L,K)=DXV(L)*VHLPF(L,K)
C        TVAR1E(L,K)=DYU(L+1)*UHLPF(L+1,K)
C        TVAR1W(L,K)=DYU(L)*UHLPF(L,K)
C       ENDDO
C      ENDDO
C
C      DO K=1,KC
C       DO L=2,LA
C        LN=LNC(L)
C        TVAR1N(L,K)=DZC(K)*TVAR1N(L,K)
C        TVAR1S(L,K)=DZC(K)*TVAR1S(L,K)
C        TVAR1E(L,K)=DZC(K)*TVAR1E(L,K)
C        TVAR1W(L,K)=DZC(K)*TVAR1W(L,K)
C       ENDDO
C      ENDDO
C
C
...
C       DO L=2,LA
C        LN=LNC(L)
C         WLPF(L,K)=SWB(L)*( WLPF(L,K-1)
C     &      +TVAR1W(L,K)-TVAR3W(L)
C     &      -TVAR1E(L,K)+TVAR3E(L)
C     &      +TVAR1S(L,K)-TVAR3S(L)
C     &      -TVAR1N(L,K)+TVAR3N(L)
C     &      +QSUMLPF(L,K)-TVAR2C(L,K) )
C       ENDDO
C      ENDDO
C      ENDIF
C
