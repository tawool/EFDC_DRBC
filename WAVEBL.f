...
!
!----------------------------------------------------------------------C
!mlt +++++ modification for SWAN 40.11 input to be verified.
!**********************************************************************C
!
      INCLUDE 'EFDC.PAR'
      INCLUDE 'EFDC.CMN'
...
!mlt      READ(1,*,IOSTAT=ISO)IWVH,IWDIR,IWPRD1,IWPRD2
      IF(ISO.GT.0) GOTO 1083
      HMPW(L)=HMP(L)
	WVWHA(L)= wwvh
!mlt      WVWHA(L)=0.001*FLOAT(IWVH)
	WACCWE(L)= wwdir/57.29578
!mlt      WACCWE(L)=FLOAT(IWDIR)/57.29578
	IF(wwprdp.GT.0.) then
		WVFRQL(L)= wwprdp
	else
		WVFRQL(L)= 1.
...
!mlt      IF(IWPRD2.GT.0) WVFRQL(L)=0.01*FLOAT(IWPRD2)
!mlt      IF(IWPRD2.LE.0) WVFRQL(L)=1.
...
!      WRITE(8,666)NTMP,FNWAVE   !hnr 7/27/2009
!
      JSWAVE=1
      ITWCBL1=1
      ITWCBL2=0
...
!**********************************************************************C
!
! ** UPDATE WAVE FIELD AT NWUPDT INTERVALS
! 
  100 CONTINUE
!
      JSWRPH=0
!
      IF(JSWAVE.EQ.1)THEN
        IF(NWCUNT.EQ.NWUPDT)THEN
          ...
          CFNWAVE(8)='8'
          CFNWAVE(9)='9'
          FNWAVE='WV' // CFNWAVE(ITWCBL3) // CFNWAVE(ITWCBL2) // CFNWAVE(ITWCBL1)// '.INP'
        ELSE
          NWCUNT=NWCUNT+1
          RETURN
        ENDIF
...
!
      DO L=2,LA
	 READ(1,*,IOSTAT=ISO) wwvh, wwdir, wwprdp
!mlt      READ(1,*,IOSTAT=ISO)IWVH,IWDIR,IWPRD1,IWPRDP
      IF(ISO.GT.0) GOTO 1083
      HMPW(L)=HMP(L)
	WVWHA(L)= 0.5 * (wwvh + WVWHA(L))
!mlt      WVWHA(L)=0.5*(0.001*FLOAT(IWVH)+WVWHA(L))
	WACCWE(L)= wwdir/57.29578
!mlt      WACCWE(L)=FLOAT(IWDIR)/57.29578
	IF(wwprdp.GT.0.) then
		WVFRQL(L)= wwprdp
...
!mlt      IF(IWPRDP.GT.0) WVFRQL(L)=0.01*FLOAT(IWPRDP)
!mlt      IF(IWPRDP.LE.0) WVFRQL(L)=1.
      WVFRQL(L)=2.*PI/WVFRQL(L)
      ENDDO
!
      CLOSE(1)
...
!      WRITE(8,666)N,FNWAVE   !hnr 7/27/2009
!
...
!
!jh       IF(REYWAVE.LT.5.E5)THEN
!jh         CDTMP=0.
!jh         IF(REYWAVE.GT.0.) CDTMP=1./SQRT(REYWAVE)
!jh         QQWV2(L)=CDTMP*UWORBIT*UWORBIT
!jh       ENDIF
!
! ** TURBULENT WAVE BOUNDARY LAYER
!
!jh       IF(REYWAVE.GE.5.E5)THEN
!
...
!
!       TVAL1=SINH(WVKHP(L))
!       TVAL2=CSEDVIS(SED(L,1,1))
        WRITE(1,600)L,IL(L),JL(L),WVWHA(L),WVFRQL(L),AEXTMP,UWORBIT,VISMUD,REYWAVE,CDTMP,QQWV1(L),QQWV2(L),ZBR(L),ZBRE(L)
      ENDDO
!
       CLOSE(1)
!
