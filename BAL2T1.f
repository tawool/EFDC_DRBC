...
!
      ...
      UMOBEG=UMOBEG+SPB(L)*0.5*DXYP(L)*HP(L)*(DYIU(L)*HUI(L)*UHDYE(L)+DYIU(L+1)*HUI(L+1)*UHDYE(L+1))
...
!     UUEBEG=UUEBEG+SPB(L)*0.25*(DXYU(L)*HU(L)*U(L,K)*U(L,K)
!    &      +DXYU(L+1)*HU(L+1)*U(L+1,K)*U(L+1,K))*DZC(K)
!     VVEBEG=VVEBEG+SPB(L)*0.25*(DXYV(L)*HV(L)*V(L,K)*V(L,K)
!    &      +DXYV(LN)*HV(LN)*V(LN,K)*V(LN,K))*DZC(K)
      UUEBEG=UUEBEG+SPB(L)*0.125*DXYP(L)*HP(L)*DZC(K)*( (U(L,K)+U(L+1,K))*(U(L,K)+U(L+1,K)) )
      VVEBEG=VVEBEG+SPB(L)*0.125*DXYP(L)*HP(L)*DZC(K)*( (V(L,K)+V(LN,K))*(V(L,K)+V(LN,K)) )
      BBEBEG=BBEBEG+SPB(L)*GP*DXYP(L)*HP(L)*DZC(K)*( BELV(L)+0.5*HP(L)*(Z(K)+Z(K-1)) )*B(L,K)
      ENDDO
      ENDDO
!
!
      DO NS=1,NSED
        DO K=1,KC
        DO L=2,LA
          SEDBEG2T(NS)=SEDBEG2T(NS)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*SED(L,K,NS)
          SEDBEG2TW(NS)=SEDBEG2TW(NS)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*SED(L,K,NS)
        ENDDO
        ENDDO
      ENDDO
      DO NS=1,NSND
        DO K=1,KC
        DO L=2,LA
          SNDBEG2T(NS)=SNDBEG2T(NS)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*SND(L,K,NS)
          SNDBEG2TW(NS)=SNDBEG2TW(NS)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*SND(L,K,NS)
        ENDDO
        ENDDO
      ENDDO
      DO NT=1,NTOX
        DO K=1,KC
        DO L=2,LA
          TOXBEG2T(NT)=TOXBEG2T(NT)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*TOX(L,K,NT)
          TOXBEG2TW(NT)=TOXBEG2TW(NT)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*TOX(L,K,NT)
        ENDDO
        ENDDO
      ENDDO
!
...
!
      KK=0
!      WRITE(1,*)KK,VOLBEG2T,WVOLBEG2T
      DO L=2,LA
      DO K=1,KBT(L)
      VOLBEG2T=VOLBEG2T+SPB(L)*DXYP(L)*HBED(L,K)
      BVOLBEG2T=BVOLBEG2T+SPB(L)*DXYP(L)*HBED(L,K)
!jmh     WVOLBEG2T=WVOLBEG2T+SPB(L)*DXYP(L)*PORBED(L,K)*HBED(L,K)
!      WRITE(1,*)KK,VOLBEG2T,WVOLBEG2T,PORBED(L,K)
      ENDDO
      ENDDO
!
      CLOSE(1)
