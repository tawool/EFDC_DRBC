...
!
      ...
      INCLUDE 'EFDC.CMN'
!
      PARAMETER (SMCW2=2.739726E-5)
      DIMENSION SMKPON(NSMGM),SMKPOP(NSMGM),SMKPOC(NSMGM)
      DIMENSION SMTHKN(NSMGM),SMTHKP(NSMGM),SMTHKC(NSMGM)
      INTEGER ISSKIP
...
!XH        OPEN(INSMICI,FILE='SM-ICI.IN',STATUS='OLD')
!XH WQSDICI.INP REPLACES SM-ICI.IN
       ELSE IF(ISMICI.EQ.2)THEN
        WRITE(2,50)'* SPATIALLY/TEMPORALLY-VARYING ICS FROM WQSDRST.INP'
!XH        OPEN(INSMRST,FILE='SM-RST.IN',STATUS='OLD')
!XH WQSDRST.INP REPLACES SM-RST.IN
       ELSE
        WRITE(2,50)'* SPATIALLY/TEMPORALLY CONSTANT INITIAL CONDITIONS'
      ENDIF
      IF(ISMRST.EQ.1)THEN
        WRITE(2,50)'* WRITE SPATIAL DISTRIBUTIONS TO WQSDRST.OUT'
!XH        OPEN(ISMORST,FILE='SM-RST.OUT',STATUS='UNKNOWN')
!XH WQSDRST.OUT REPLACES SM-RST.OUT
!XH        WRITE(ISMORST,888)
!
       ELSE
...
!04
      ...
      IF(MOD(ISMTSDT,IWQDT).NE.0)STOP 'ERROR!! ISMTSDT SHOULD BE MULTIPLE OF IWQDT'
!
  999 FORMAT(1X)
 5100 FORMAT(A79)
 5101 FORMAT(10I8)
...
!
!06 SPATIALLY CONSTANT PARAMETERS FOR SPLITING DEPOSITIONAL FLUXES OF ALGAE
!
!     READ(1,999)
      WRITE(2,999)
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
...
!
      ...
      IF(ISSKIP .EQ. 0) READ(1,999)
      WRITE(2,999)
      READ(1,*) SMFNBC(1),SMFNBC(2),SMFNBC(3),SMFNBD(1),SMFNBD(2),SMFNBD(3),SMFNBG(1),SMFNBG(2),SMFNBG(3)
      ...
      IF(SUMNBC.LT.0.9999.OR.SUMNBC.GT.1.0001)STOP 'ERROR!! SMFNBC(1)+SMFNBC(2)+SMFNBC(3) SHOULD BE 1'
      IF(SUMNBD.LT.0.9999.OR.SUMNBD.GT.1.0001)STOP 'ERROR!! SMFNBD(1)+SMFNBD(2)+SMFNBD(3) SHOULD BE 1'
      IF(SUMNBG.LT.0.9999.OR.SUMNBG.GT.1.0001)STOP 'ERROR!! SMFNBG(1)+SMFNBG(2)+SMFNBG(3) SHOULD BE 1'
!07
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      IF(ISSKIP .EQ. 0) READ(1,999)
      WRITE(2,999)
      ...
      IF(SUMPBC.LT.0.9999.OR.SUMPBC.GT.1.0001)STOP 'ERROR!! SMFPBC(1)+SMFPBC(2)+SMFPBC(3) SHOULD BE 1'
      IF(SUMPBD.LT.0.9999.OR.SUMPBD.GT.1.0001)STOP 'ERROR!! SMFPBD(1)+SMFPBD(2)+SMFPBD(3) SHOULD BE 1'
      IF(SUMPBG.LT.0.9999.OR.SUMNBG.GT.1.0001)STOP 'ERROR!! SMFPBG(1)+SMFPBG(2)+SMFPBG(3) SHOULD BE 1'
!08
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      IF(ISSKIP .EQ. 0) READ(1,999)
      WRITE(2,999)
      ...
      IF(SUMCBC.LT.0.9999.OR.SUMCBC.GT.1.0001)STOP 'ERROR!! SMFCBC(1)+SMFCBC(2)+SMFCBC(3) SHOULD BE 1'
      IF(SUMCBD.LT.0.9999.OR.SUMCBD.GT.1.0001)STOP 'ERROR!! SMFPBD(1)+SMFCBD(2)+SMFCBD(3) SHOULD BE 1'
      IF(SUMCBG.LT.0.9999.OR.SUMCBG.GT.1.0001)STOP 'ERROR!! SMFCBG(1)+SMFCBG(2)+SMFCBG(3) SHOULD BE 1'
!
...
!
!11 SPATIALLY CONSTANT PARAMETERS COMMON FOR SEDIMENT FLUX
!
      WRITE(2,999)
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      READ(1,5100) TITLE(1)
...
!
!13 SPATIALLY CONSTANT PARAMETERS FOR NH4, NO3 & PO4 FLUX
!
      WRITE(2,999)
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      READ(1,5100) TITLE(1)
      ...
      WRITE(2,52)'* PARTITION COEF FOR PO4 IN LAYER 2 (L/KG) = ',SMP2PO4,': CRITICAL DO (MG/L) FOR PO4 SORPTION      = ',SMCO2PO4
...
!
!14 SPATIALLY CONSTANT PARAMETERS FOR H2S/CH4 FLUX AND SOD
!
      WRITE(2,999)
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      READ(1,5100) TITLE(1)
      ...
      WRITE(2,52)'* TEMPERATURE EFFECT ON H2S OXIDATION RATE = ',SMTHH2S,': OXYGEN EFFECT (MG/L) ON H2S OXIDATION    = ',SMKMH2S
      WRITE(2,52)'* METHANE OXIDATION REACTION VELOCITY (M/D)= ',SMKCH4,': TEMPERATURE EFFECT ON CH4 OXIDATION RATE = ',SMTHCH4
...
!
!16 SPATIALLY CONSTANT PARAMETERS FOR SILICA
!
      WRITE(2,999)
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      READ(1,5100) TITLE(1)
     ...
     *          ,'* DISSOLUTION HALF-SAT CONSTANT (G SI/M^3) = ',SMKMPSI
      ...
      SMFP2SI = 1.0 - SMFD2SI
!
! SET UP LOOK-UP TABLE FOR TEMPERATURE DEPENDENCY OVER -5OC TO 35OC
!
      DO IT=1,NWQTD
        STEMP = REAL(IT-1)*0.1 - 4.95
!        STEMP = REAL(M-1)*0.1 - 4.95
        TT20 = STEMP-20.0
        DO M=1,3
      ...
      ENDDO
!
!17 SPATIALLY CONSTANT ICS FOR SEDIMENT STATE VARIABLES (G/M^3)
!
      WRITE(2,998)
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      READ(1,5100) TITLE(1)
      ...
      READ(1,5100) TITLE(1)
      WRITE(2,5100) TITLE(1)
      READ(1,*) SMPON(1,1),SMPON(1,2),SMPON(1,3),SMPOP(1,1),SMPOP(1,2),SMPOP(1,3),SMPOC(1,1),SMPOC(1,2),SMPOC(1,3)
      IF(ISMICI.NE.1 .AND. ISMICI.NE.2)WRITE(2,5105) SMPON(1,1),SMPON(1,2),SMPON(1,3),SMPOP(1,1),SMPOP(1,2),SMPOP(1,3),SMPOC(1,1),SMPOC(1,2),SMPOC(1,3)
!18
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      READ(1,5100) TITLE(1)
      WRITE(2,5100) TITLE(1)
      ...
      ENDIF
!
!19 SPATIALLY VARYING PHYSICAL & RATE VELOCITY: CONVERT W2 FROM CM/YR TO M/D
!   SMDIFT IN M^2/D
!
      WRITE(2,998)
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      ...
      ENDDO
!
!20 SPATIALLY VARYING PARAMETERS: DISTRIBUTION COEFFICIENTS FOR RPOM
!
      WRITE(2,998)
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      READ(1,5100) TITLE(1)
...
!TT       IF(SMFNR(I,1)+SMFNR(I,2)+SMFNR(I,3).NE.1.0)THEN
!TT         PRINT*, 'AT SEGMENT NUMBER ', I
!TT         STOP 'ERROR!! SMFNR(I,1)+)SMFNR(I,2)+SMFNR(I,3) SHOULD BE 1'
!TT       ENDIF
!TT       IF(SMFPR(I,1)+SMFPR(I,2)+SMFPR(I,3).NE.1.0)THEN
!TT         PRINT*, 'AT SEGMENT NUMBER ', I
!TT         STOP 'ERROR!! SMFPR(I,1)+SMFPR(I,2)+SMFPR(I,3) SHOULD BE 1'
!TT       ENDIF
!TT       IF(SMFCR(I,1)+SMFCR(I,2)+SMFCR(I,3).NE.1.0)THEN
!TT         PRINT*, 'AT SEGMENT NUMBER ', I
!TT         STOP 'ERROR!! SMFCR(I,1)+SMFCR(I,2)+SMFCR(I,3) SHOULD BE 1'
!TT       ENDIF
        SUMNBC=SMFNR(I,1)+SMFNR(I,2)+SMFNR(I,3)
        SUMNBD=SMFPR(I,1)+SMFPR(I,2)+SMFPR(I,3)
        SUMNBG=SMFCR(I,1)+SMFCR(I,2)+SMFCR(I,3)
        IF(SUMNBC.LT.0.9999.OR.SUMNBC.GT.1.0001)STOP 'ERROR!! SMFNR(I,1)+SMFNR(I,2)+SMFNR(I,3) SHOULD BE 1'
        IF(SUMNBD.LT.0.9999.OR.SUMNBD.GT.1.0001)STOP 'ERROR!! SMFPR(I,1)+SMFPR(I,2)+SMFPR(I,3) SHOULD BE 1'
        IF(SUMNBG.LT.0.9999.OR.SUMNBG.GT.1.0001)STOP 'ERROR!! SMFCR(I,1)+SMFCR(I,2)+SMFCR(I,3) SHOULD BE 1'
      ENDDO
!
      CLOSE(1)
!      WRITE(6,6666)'READ WQ3DSD.INP'
 6666 FORMAT(A30)
!
  998 FORMAT(80X)
 5105 FORMAT(10F8.2)
