...
C 01/02/2002        John Hamrick       01/02/2002       John Hamrick
...
C  open boundaries
C 01/23/2002        John Hamrick       01/23/2002       John Hamrick
...
C**********************************************************************C
C
      DIMENSION TMPVEC1(KCM),TMPVEC2(KCM)
...
C
      IF(IS2LMC.EQ.1)THEN
        DO L=2,LA
          ...
          LN=LNC(L)
          LS=LSC(L)               
          UHC1=0.5*(UHDY2(L,1)+UHDY2(LS,1))
          UHB1=0.5*(UHDY2(L,1)+UHDY2(L+1,1))
          VHC1=0.5*(VHDX2(L,1)+VHDX2(L-1,1))
          VHB1=0.5*(VHDX2(L,1)+VHDX2(LN,1))
          UHC2=0.5*(UHDY2(L,2)+UHDY2(LS,2))
          UHB2=0.5*(UHDY2(L,2)+UHDY2(L+1,2))
          VHC2=0.5*(VHDX2(L,2)+VHDX2(L-1,2))
          VHB2=0.5*(VHDX2(L,2)+VHDX2(LN,2))
C
          UHB1MX=0.
          UHB1MN=0.
          VHC1MX=0.
...
C
     ...
     &   UHB1MX=1.+CK2UUM*(UHB2-UHB1)*(U(L,2)-U(L,1))/UHB1*U(L,1)
       ...
       IF(BOTT.GT.0.0)
     &   UHB1MN=1.+CK2UUM*(UHB2-UHB1)*(U(L+1,2)-U(L+1,1))/UHB1*U(L+1,1)
     ...
     &   VHC1MN=1.+CK2UVM*(VHC2-VHC1)*(U(L,2)-U(L,1))/VHC1*U(L,1)
       ...
       IF(BOTT.GT.0.0)
     &   UHC1MX=1.+CK2UVM*(UHC2-UHC1)*(V(L-1,2)-V(L-1,1))/UHC1*V(L-1,1)
       BOTT=ABS(UHC1*V(L,1))
       IF(BOTT.GT.0.0)
     ...
     &   UHB2MX=1.+CK2UUM*(UHB2-UHB1)*(U(L,2)-U(L,1))/UHB2*U(L,2)
       ...
       IF(BOTT.GT.0.0)
     &   UHB2MN=1.+CK2UUM*(UHB2-UHB1)*(U(L+1,2)-U(L+1,1))/UHB2*U(L+1,2)
     ...
     &   VHC2MN=1.+CK2UVM*(VHC2-VHC1)*(U(L,2)-U(L,1))/VHC2*U(L,2)
       ...
       IF(BOTT.GT.0.0)
     &   UHC2MX=1.+CK2UVM*(UHC2-UHC1)*(V(L-1,2)-V(L-1,1))/UHC2*V(L-1,2)
       BOTT=ABS(UHC2*V(L,2))
       IF(BOTT.GT.0.0)
...
C----------------------------------------------------------------------C
C
C **  CALCULATION FOR 2 LAYERS WITH MOMENTUM-CURVATURE CORRECTION
C
      IF(IS2LMC.EQ.1)THEN
C     IF(KC.EQ.2)THEN
       DO L=2,LA
...
C
C      NTMPVAL=MOD(N,8)
C      IF(ISTL.EQ.3.AND.NTMPVAL.EQ.0)THEN
C        OPEN(1,FILE='VEGEIMP.DIA')
C        CLOSE(1,STATUS='DELETE')
C        OPEN(1,FILE='VEGEIMP.DIA')
C        DO L=2,LA
C         WRITE(1,1947)N,IL(L),JL(L),FXVEGE(L),(FXVEG(L,K),K=1,KC)
C         WRITE(1,1948)FYVEGE(L),(FYVEG(L,K),K=1,KC)
C        ENDDO
C        CLOSE(1)
C      ENDIF
C
...
C
C      CWALL=0.0
C      IF(ISHDMF.EQ.2) CWALL=0.01
C
C      DO K=1,KC
C       DO L=2,LA
C        LN=LNC(L)
C        LNW=LNWC(L)
C        IF(SVBO(LN).LE.0.5.OR.SVBO(LNW).LE.0.5)THEN
C          TMPVAL=1.+SUBO(L)+SVBO(LN)+SVBO(LNW)
C          HCN=(HP(L)+SUBO(L)*HP(L-1)+SVBO(LN)*HP(LN)
C     &               +SVBO(LNW)*H1P(LNW))/TMPVAL
C          FMDUYN=DXU(L)*HCN*CWALL*ABS(U(L,K))*U(L,K)
C          FX(L,K)=FX(L,K)
C     &        -SDX(L)*(FMDUX(L,K)-FMDUX(L-1,K)+FMDUYN-FMDUY(L,K))
C         ELSE
C          FX(L,K)=FX(L,K)
...
C        LS=LSC(L)
C        LSE=LSEC(L)
C        IF(SUB(L+1).LE.0.5.OR.SUB(LSE).LE.0.5)THEN
C          TMPVAL=1.+SUBO(L+1)+SVBO(L)+SUBO(LSE)
C          HCE=(HP(L)+SUBO(L+1)*HP(L+1)+SVBO(L)*HP(LS)
C     &         +SVBO(LSE)*H1P(LSE))/TMPVAL
C          FMDVXE=DYV(L)*HCE*CWALL*ABS(V(L,K))*V(L,K)
C          FY(L,K)=FY(L,K)
C     &        -SDY(L)*(FMDVXE-FMDVX(L,K)+FMDVY(L,K)-FMDVY(LS,K))
C         ELSE
C          FY(L,K)=FY(L,K)
...
C
      DO K=1,KC
       DO L=2,LA
         IF(LMASKDRY(L))THEN
          FX(L,K)=FX(L,K)
     &        -SDX(L)*(FMDUX(L,K)+FMDUY(L,K))
          FY(L,K)=FY(L,K)
     &        -SDY(L)*(FMDVX(L,K)+FMDVY(L,K))
         ENDIF
       ENDDO
      ENDDO
C
...
C
      ...
      DO L=2,LA
      LS=LSC(L)
C      FBBX(L,K)=SBX(L)*GP*HU(L)*
      FBBX(L,K)=ROLDPG*FBBX(L,K)+RNEWPG*SBX(L)*GP*HU(L)*
     &            ( HU(L)*( (B(L,K+1)-B(L-1,K+1))*DZC(K+1)
     &                     +(B(L,K)-B(L-1,K))*DZC(K) )
     &           -(B(L,K+1)-B(L,K)+B(L-1,K+1)-B(L-1,K))*
     &            (BELV(L)-BELV(L-1)+Z(K)*(HP(L)-HP(L-1))) )
C      FBBY(L,K)=SBY(L)*GP*HV(L)*
      FBBY(L,K)=ROLDPG*FBBY(L,K)+RNEWPG*SBY(L)*GP*HV(L)*
...
C
      ...
      LS=LSC(L)
      FBBX(L,K)=SBX(L)*GP*HU(L)*
     &            ( 0.5*HU(L)*( 0.5*(B(L,K+2)-B(L-1,K+2))*DZC(K+2)
     &                         +1.5*(B(L,K+1)-B(L-1,K+1))*DZC(K+1)
     &                         +2.0*(B(L,K  )-B(L-1,K  ))*DZC(K  ) )
     &           -0.25*(B(L,K+2)-B(L,K+1)+B(L-1,K+2)-B(L-1,K+1))*
     &            (BELV(L)-BELV(L-1)+Z(K+1)*(HP(L)-HP(L-1)))
     &           -0.50*(B(L,K+2)-B(L,K+1)+B(L-1,K+2)-B(L-1,K+1))*
     &            (BELV(L)-BELV(L-1)+Z(K+1)*(HP(L)-HP(L-1))) )
      FBBY(L,K)=SBY(L)*GP*HV(L)*
     &            ( 0.5*HV(L)*( 0.5*(B(L,K+2)-B(LS ,K+2))*DZC(K+2)
     &                         +1.5*(B(L,K+1)-B(LS ,K+1))*DZC(K+1) 
     &                         +2.0*(B(L,K  )-B(LS ,K  ))*DZC(K  ) )
...
C
      ...
      LS=LSC(L)
      FBBX(L,K)=SBX(L)*GP*HU(L)*
     &            ( 0.5*HU(L)*( 2.0*(B(L,K+1)-B(L-1,K+1))*DZC(K+1)
     &                         +1.5*(B(L,K  )-B(L-1,K  ))*DZC(K  )
     &                         +0.5*(B(L,K-1)-B(L-1,K-1))*DZC(K-1) )
     &            -0.50*(B(L,K+1)-B(L,K+1)+B(L-1,K+1)-B(L-1,K+1))*
     &            (BELV(L)-BELV(L-1)+Z(K+1)*(HP(L)-HP(L-1)))
     &            -0.25*(B(L,K  )-B(L,K-1)+B(L-1,K  )-B(L-1,K-1))*
     &            (BELV(L)-BELV(L-1)+Z(K-1)*(HP(L)-HP(L-1))) )
      FBBY(L,K)=SBY(L)*GP*HV(L)*
     &            ( 0.5*HV(L)*( 2.0*(B(L,K+1)-B(LS ,K+1))*DZC(K+1)
     &                         +1.5*(B(L,K  )-B(LS ,K  ))*DZC(K  ) 
     &                         +0.5*(B(L,K-1)-B(LS ,K-1))*DZC(K-1) )
     &            -0.50*(B(L,K+1)-B(L,K  )+B(LS ,K+1)-B(LS ,K  ))*
     &            (BELV(L)-BELV(LS)+Z(K  )*(HP(L)-HP(LS)))
