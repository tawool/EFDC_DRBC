...
C
C  888 FORMAT('    L',
C     *'       GPON1       GPON2       GPON3       GPOP1       GPOP2',
C     *'       GPOP3       GPOC1       GPOC2       GPOC3       G1NH4',
C     *'       G2NH4       G2NO3       G2PO4       G2H2S        GPSI',
C     *'        G2SI        GBST          GT')
C03
      IF(ISSKIP .GT. 0) CALL SKIPCOMM(1,CCMRM)
      IF(ISSKIP .EQ. 0) READ(1,999)
...
C04
     ...
     *  ISMTSDT
      IF(MOD(ISMTSDT,IWQDT).NE.0)
     *  STOP 'ERROR!! ISMTSDT SHOULD BE MULTIPLE OF IWQDT'
C
  999 FORMAT(1X)
 5100 FORMAT(A79)
 5101 FORMAT(10I8)
...
C
     ...
     *    SMFPR(I,2),SMFPR(I,3),SMFCR(I,1),SMFCR(I,2),SMFCR(I,3)
CTT       IF(SMFNR(I,1)+SMFNR(I,2)+SMFNR(I,3).NE.1.0)THEN
CTT         PRINT*, 'AT SEGMENT NUMBER ', I
CTT         STOP 'ERROR!! SMFNR(I,1)+)SMFNR(I,2)+SMFNR(I,3) SHOULD BE 1'
CTT       ENDIF
CTT       IF(SMFPR(I,1)+SMFPR(I,2)+SMFPR(I,3).NE.1.0)THEN
CTT         PRINT*, 'AT SEGMENT NUMBER ', I
CTT         STOP 'ERROR!! SMFPR(I,1)+SMFPR(I,2)+SMFPR(I,3) SHOULD BE 1'
CTT       ENDIF
CTT       IF(SMFCR(I,1)+SMFCR(I,2)+SMFCR(I,3).NE.1.0)THEN
CTT         PRINT*, 'AT SEGMENT NUMBER ', I
CTT         STOP 'ERROR!! SMFCR(I,1)+SMFCR(I,2)+SMFCR(I,3) SHOULD BE 1'
CTT       ENDIF
...
C
      CLOSE(1)
C      WRITE(6,6666)'READ WQ3DSD.INP'
 6666 FORMAT(A30)
C
  998 FORMAT(80X)
 5105 FORMAT(10F8.2)
...
C
        CLOSE(1)
C        WRITE(6,6666)'READ WQSDMAP.INP'
C
      ENDIF
C
      CLOSE(2)
