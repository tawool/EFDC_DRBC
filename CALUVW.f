...
chnr      DO K=1,KC
chnr      DO L=2,LA
chnr       CFLUUUT=DELT*ABS(DXIU(L)*U(L,K))
chnr       CFLUUU(L,K)=MAX(CFLUUUT,CFLUUU(L,K))
chnr       CFLVVVT=DELT*ABS(DYIV(L)*V(L,K))
chnr       CFLVVV(L,K)=MAX(CFLVVVT,CFLVVV(L,K))
chnr       CFLWWWT=DELT*ABS(HPI(L)*DZIG(K)*W(L,K))
chnr       CFLWWW(L,K)=MAX(CFLWWWT,CFLWWW(L,K))
chnr       CFLCACT=DELT*ABS(CAC(L,K)*DXYIP(L)*HPI(L))
chnr       CFLCAC(L,K)=MAX(CFLCACT,CFLCAC(L,K))
chnr      ENDDO
chnr      ENDDO
      DO K=1,KC                                               !hnr
      DO L=2,LA                                               !hnr
       CFLUUU(L,K)=DELT*ABS(DXIU(L)*U(L,K))                   !hnr
       CFLVVV(L,K)=DELT*ABS(DYIV(L)*V(L,K))                   !hnr
       CFLWWW(L,K)=DELT*ABS(HPI(L)*DZIG(K)*W(L,K))            !hnr
       CFLCAC(L,K)=DELT*ABS(CAC(L,K)*DXYIP(L)*HPI(L))         !hnr
      ENDDO                                                   !hnr
      ENDDO                                                   !hnr
C
C**********************************************************************C
