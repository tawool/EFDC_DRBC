...
!**********************************************************************C
!
!  CONTROL SUBROUTINE FOR SEDIMENT COMPONENT OF WATER QUALITY MODEL
!
!  ORGINALLY CODED BY K.-Y. PARK
!  OPTIMIZED AND MODIFIED BY J. M. HAMRICK
!  LAST MODIFIED BY M.R. MORTON  2 JUNE 1999
! 06/02/99 MRM: ADDED BFO2SUM, BFNH4SUM, BFNO3SUM, BFPO4SUM, BFCODSUM,
!   AND BFSADSUM ARRAYS TO KEEP TRACK OF BENTHIC FLUXES AT ALL CELLS
!   FOR LATER STORAGE IN BINARY FILE (WQSDTS.BIN).
!
!**********************************************************************C
!
      INCLUDE 'EFDC.PAR'
      INCLUDE 'EFDC.CMN'
!
      COMMON/TSM/SMW12(LCM),SMKL12(LCM),XSMO20(LCM),SMTMP(LCM),ISMT(LCM)
!
!**********************************************************************C
!
! SED TEMP., & FIND AN INDEX FOR LOOK-UP TABLE FOR TEMPERATURE
!  DEPENDENCY
!: SM1DIFT(IZ)=SMDIFT*SMDTOH/SMHSED,SM2DIFT=1/(1+SM1DIFT)
!
      DO L=2,LA
        IZ = ISMZMAP(L)
...
! MRM +++++++++ ADDED BY M. MORTON 07/24/98
...
911       FORMAT(/,'ERROR!! INVALID SEDIMENT TEMPERATURE',/,'TIME, L, I, J, TEM(L,1), SMT(L) = ', F10.5, 3I4, 2F10.4)
          CLOSE(1)
! MRM +++++++++ ADDED BY M. MORTON 07/24/98
          PRINT*, 'L, TEM(L,1), SMT(L) = ', L,TEM(L,1),SMT(L)
!
! MRM: THE FOLLOWING STOP WAS COMMENTED OUT BY M. MORTON 07/24/98 AND
...
!          STOP 'ERROR!! INVALID SEDIMENT TEMPERATURE'
      ...
      ENDDO
!
!1 DEPOSITIONAL FLUX FROM WATER COLUMN (SMDFN,SMDFP,SMDFC)
!: SMHODT(IZ)=SMHSED/DTWQ,SMDTOH(IZ)=DTWQ/SMHSED
!
      DO M=1,NSMG
        DO L=2,LA
          ...
          SMDFCA = SMFCBC(M)*WQDFBC(L) + SMFCBD(M)*WQDFBD(L) + SMFCBG(M)*WQDFBG(L)
...
! J.S. ADDED LABILE PORTION
      ...
      ENDDO
!
!2 DIAGENESIS EQ AND DIAGENESIS FLUX (SMDGFN,SMDGFP,SMDGFC)
!: SMW2 IN M/D,SMW2DTOH(IZ)=1.0+SMW2*SMDTOH
!
      DO M=1,NSMG
        DO L=2,LA
          SMPON(L,M)=SCB(L)*(SMPON(L,M) + SMDFN(L,M)*SMDTOH(ISMZMAP(L)))/ (SMW2DTOH(ISMZMAP(L)) + SMTDND(ISMT(L),M)*DTWQ+ 1.E-18)
          SMPOP(L,M)=SCB(L)*(SMPOP(L,M) + SMDFP(L,M)*SMDTOH(ISMZMAP(L)))/ (SMW2DTOH(ISMZMAP(L)) + SMTDPD(ISMT(L),M)*DTWQ+ 1.E-18)
          SMPOC(L,M)=SCB(L)*(SMPOC(L,M) + SMDFC(L,M)*SMDTOH(ISMZMAP(L)))/ (SMW2DTOH(ISMZMAP(L)) + SMTDCD(ISMT(L),M)*DTWQ+ 1.E-18)
        ENDDO
      ENDDO
!
      DO ND=1,NDMWQ
      ...
      LL=LF+LDM-1
      DO L=LF,LL
        SMDGFN(L) = SCB(L)*SMHSED(ISMZMAP(L)) * (SMTDND(ISMT(L),1)*SMPON(L,1) + SMTDND(ISMT(L),2)*SMPON(L,2))
        SMDGFP(L) = SCB(L)*SMHSED(ISMZMAP(L)) * (SMTDPD(ISMT(L),1)*SMPOP(L,1) + SMTDPD(ISMT(L),2)*SMPOP(L,2))
        SMDGFC(L) = SCB(L)*SMHSED(ISMZMAP(L)) * (SMTDCD(ISMT(L),1)*SMPOC(L,1) + SMTDCD(ISMT(L),2)*SMPOC(L,2))
!
!3 MASS-BALANCE EQ'S FOR FLUXES
!
! COMMON PARAMETERS: SMBST1=1/(1+SMKBST*DTWQ),SM1OKMDP=1/SMKMDP
!: ISMTDMBS=NINT(SMTDMBS/DTWQ),ISMTCMBS=NINT(SMTCMBS/DTWQ)
!: USE SMTMP(L) TO STORE OLD SMBST(L)
!
!MRM MODIFIED FOLLOWING LINE TO PREVENT HIGH FLUX OF PO4 DURING LOW
!MRM D.O. EVENTS:
!MRM     XSMO20(L) = MAX( WQV(L,1,19), 0.1 )
        XSMO20(L) = MAX( WQV(L,1,19), 3.0 )
        SMTMP(L) = SCB(L)*SMBST(L)
        IF(XSMO20(L).LT.SMKMDP)THEN
          SMBST(L) =SCB(L)*(SMTMP(L)+DTWQ*(1.0-XSMO20(L)*SM1OKMDP)) * SMBST1
         ELSE
...
!
      ...
      ENDIF
!
!: SMDP(IZ)=SMDP/(SMHSED*SMPOCR),SMDD(IZ)=SMDD/SMHSED
...
!
      ...
      ENDDO
!
! NH4, NO3: SMKMO2N=SMKMO2N*2,SMKNH4=SMKNH4^2*SMKMNH4,
!: SMW2PHODT(IZ)=SMW2+SMHODT,SMK1NO3=SMK1NO3^2
!
      DO L=2,LA
      IF(SCB(L).GT.0.5)THEN
        IZ = ISMZMAP(L)
        SMO20 = XSMO20(L)
        SK1NH4SM = ( SMKNH4(IZ)*SMTDNH4(ISMT(L)) * SMO20 ) / ( (SMKMO2N+SMO20+ 1.E-12) * (SMKMNH4+SM1NH4(L)) )
...
!
! H2S/CH4
!: SMK1H2S=(SMKD1HS^2*SMFD1H2S+SMKP1HS^2*SMFP1H2S)/(2*SMKMH2S)*SMTHH2S**(T-20)
!: SMTD1CH4(IT)=SMTD1CH4*20
!
...
!
        ...
        SM2H2S(L) = RSM2H2S
!        WQBFO2(L) = -SMSOD
! MRM  SODMULT IS A SPATIALLY VARIABLE ADJUSTMENT FACTOR FOR SOD:
...
!       WQBFCOD(L) = SMJAQH2S(L)
!        WQBFCOD(L) = AQJH2SSM-SMSS(L)*WQV(L,1,18) + AQJCH4SM
!
      ENDIF
      ENDDO
!
...
!
      ...
      ENDIF
!
! KEEP TRACK OF BENTHIC FLUX RATES HERE FOR LATER SAVE TO BINARY FILE:
!
      DO L=2,LA
