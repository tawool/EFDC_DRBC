...
!
      DO NS=1,NSED
        DO K=1,KC
        DO L=2,LA
          SEDEND2T(NS)=SEDEND2T(NS)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*SED(L,K,NS)
          SEDEND2TW(NS)=SEDEND2TW(NS)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*SED(L,K,NS)
        ENDDO
        ENDDO
      ENDDO
      DO NS=1,NSND
        DO K=1,KC
        DO L=2,LA
          SNDEND2T(NS)=SNDEND2T(NS)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*SND(L,K,NS)
          SNDEND2TW(NS)=SNDEND2TW(NS)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*SND(L,K,NS)
        ENDDO
        ENDDO
      ENDDO
      DO NT=1,NTOX
        DO K=1,KC
        DO L=2,LA
          TOXEND2T(NT)=TOXEND2T(NT)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*TOX(L,K,NT)
          TOXEND2TW(NT)=TOXEND2TW(NT)+SCB(L)*DXYP(L)*HP(L)*DZC(K)*TOX(L,K,NT)
        ENDDO
        ENDDO
      ENDDO
!
...
!jmh013103      WVOLEND2T=WVOLEND2T+SPB(L)*DXYP(L)*PORBED(L,K)*HBED(L,K)
      ENDDO
      ENDDO
!
...
! MODIFY TOXOUT2T TO INCLUDE BED LOAD BOUNDARY OUT
      ...
      ENDDO
!
! SEDOUT2T(NS) IS IS NET COHESIVE MASS GOING OUT OF DOMAIN DUE
!   TO WATER COLUMN VOLUME SOURCES AND SINKS
! SEDFLUX2T(NS) IS IS NET COHESIVE MASS FLUX POSITIVE FROM BED
!   TO WATER COLUMN
!
      DO NS=1,NSED
      ...
      ENDDO
!
!  SNDOUT2T(NS) IS NET NONCOHESIVE MASS GOING OUT OF DOMAIN DUE
!    TO WATER COLUMN VOLUME SOURCES AND SINKS
!  SBLOUT2T(NS) IS NET NONCOHESIVE SEDIMENT MASS GOING OUT OF DOMAIN DUE
!    DUE TO BED LOAD TRANSPORT OUT OF DOMAIN
!  SNDFLUX2T(NS) IS NET NONCOHESIVE SEDIMENT FLUX DUE TO SUSPENDED LOAD
!    (POSITIVE INTO WATER COLUMN)
!  SNDFBL2T(NS) IS NET NONCOHESIVE SEDIMENT FLUX FROM BED ASSOCIATED WITH
...
!      DO NS=1,NSED
!        IF(SEDEND2T(NS).NE.0.)
!     &    RSEDERE2T(NS)=SEDERR2T(NS)/SEDEND2T(NS)
!        IF(SEDEND2TW(NS).NE.0.)
!     &    RSEDERE2TW(NS)=SEDERR2TW(NS)/SEDEND2TW(NS)
!        IF(SEDEND2TB(NS).NE.0.)
!     &    RSEDERE2TB(NS)=SEDERR2TB(NS)/SEDEND2TB(NS)
!      ENDDO
!      DO NS=1,NSND
!        IF(SNDEND2T(NS).NE.0.)
!     &    RSNDERE2T(NS)=SNDERR2T(NS)/SNDEND2T(NS)
!        IF(SNDEND2TW(NS).NE.0.)
!     &    RSNDERE2TW(NS)=SNDERR2TW(NS)/SNDEND2TW(NS)
!        IF(SNDEND2TB(NS).NE.0.)
!     &    RSNDERE2TB(NS)=SNDERR2TB(NS)/SNDEND2TB(NS)
!      ENDDO
!      DO NT=1,NTOX
!        IF(TOXEND2T(NT).NE.0.)
!     &    RTOXERE2T(NT)=TOXERR2T(NT)/TOXEND2T(NT)
!        IF(TOXEND2TW(NT).NE.0.)
!     &    RTOXERE2TW(NT)=TOXERR2TW(NT)/TOXEND2TW(NT)
!        IF(TOXEND2TB(NT).NE.0.)
!     &    RTOXERE2TB(NT)=TOXERR2TB(NT)/TOXEND2TB(NT)
!      ENDDO
!
!
      DO NS=1,NSED
        TMPVAL=0.5*(SEDEND2T(NS)+SEDBEG2T(NS))
        IF(TMPVAL.NE.0.)RSEDERE2T(NS)=SEDERR2T(NS)/TMPVAL
        TMPVAL=0.5*(SEDEND2TW(NS)+SEDBEG2TW(NS))
        IF(TMPVAL.NE.0.)RSEDERE2TW(NS)=SEDERR2TW(NS)/TMPVAL
        TMPVAL=0.5*(SEDEND2TB(NS)+SEDBEG2TB(NS))
        IF(TMPVAL.NE.0.)RSEDERE2TB(NS)=SEDERR2TB(NS)/TMPVAL
      ENDDO
      DO NS=1,NSND
        TMPVAL=0.5*(SNDEND2T(NS)+SNDBEG2T(NS))
        IF(TMPVAL.NE.0.)RSNDERE2T(NS)=SNDERR2T(NS)/TMPVAL
        TMPVAL=0.5*(SNDEND2TW(NS)+SNDBEG2TW(NS))
        IF(TMPVAL.NE.0.)RSNDERE2TW(NS)=SNDERR2TW(NS)/TMPVAL
        TMPVAL=0.5*(SNDEND2TB(NS)+SNDBEG2TB(NS))
        IF(TMPVAL.NE.0.)RSNDERE2TB(NS)=SNDERR2TB(NS)/TMPVAL
      ENDDO
      DO NT=1,NTOX
        TMPVAL=0.5*(TOXEND2T(NT)+TOXBEG2T(NT))
        IF(TMPVAL.NE.0.)RTOXERE2T(NT)=TOXERR2T(NT)/TMPVAL
        TMPVAL=0.5*(TOXEND2TW(NT)+TOXBEG2TW(NT))
        IF(TMPVAL.NE.0.)RTOXERE2TW(NT)=TOXERR2TW(NT)/TMPVAL
        TMPVAL=0.5*(TOXEND2TB(NT)+TOXBEG2TB(NT))
        IF(TMPVAL.NE.0.)RTOXERE2TB(NT)=TOXERR2TB(NT)/TMPVAL
      ENDDO
!
...
!
      DO NS=1,NSED
        IF(SEDOUT2T(NS).NE.0.)RSEDERO2T(NS)=SEDERR2T(NS)/SEDOUT2T(NS)
        IF(SEDOUT2TW(NS).NE.0.)RSEDERO2TW(NS)=SEDERR2TW(NS)/SEDOUT2TW(NS)
        IF(SEDOUT2TB(NS).NE.0.)RSEDERO2TB(NS)=SEDERR2TB(NS)/SEDOUT2TB(NS)
      ENDDO
      DO NS=1,NSND
        IF(SNDOUT2T(NS).NE.0.)RSNDERO2T(NS)=SNDERR2T(NS)/SNDOUT2T(NS)
        IF(SNDOUT2TW(NS).NE.0.)RSNDERO2TW(NS)=SNDERR2TW(NS)/SNDOUT2TW(NS)
        IF(SNDOUT2TB(NS).NE.0.)RSNDERO2TB(NS)=SNDERR2TB(NS)/SNDOUT2TB(NS)
      ENDDO
      DO NT=1,NTOX
        IF(TOXOUT2T(NT).NE.0.)RTOXERO2T(NT)=TOXERR2T(NT)/TOXOUT2T(NT)
        IF(TOXOUT2TW(NT).NE.0.)RTOXERO2TW(NT)=TOXERR2TW(NT)/TOXOUT2TW(NT)
        IF(TOXOUT2TB(NT).NE.0.)RTOXERO2TB(NT)=TOXERR2TB(NT)/TOXOUT2TB(NT)
      ENDDO
!
!**********************************************************************C
!
...
!
      ...
      WRITE(89,900)
      WRITE(89,954)
      WRITE(89,960)VOLBEG2T,VOLOUT,VOLBMO2T,VOLEND2T,VOLERR2T,RVERDE2T,RVERDO2T,SBLOUT2TT
      WRITE(89,900)
!      WRITE(89,*)' NEW WATER VOLUME (W_COL+BED WATER) VOLUME BALANCE'
      WRITE(89,*)' NEW WATER VOLUME (W_COL,WATER+SED) VOLUME BALANCE'
!        WRITE(89,*)' _BEG_ = BEGINNING GLOBAL WATER VOLUME'
      WRITE(89,*)' _BEG_ = BEGINNING GLOBAL W_COL VOLUME'
!       WRITE(89,*)' _OUT_ = NET WATER VOLUME GOING OUT OF DOMAIN'
      WRITE(89,*)' _OUT_ = NET W_COL VOLUME GOING OUT OF DOMAIN'
      WRITE(89,*)' _BMO_ = BEGINNING - OUT'
!         WRITE(89,*)' _END_ = ENDING GLOBAL WATER VOLUME'
