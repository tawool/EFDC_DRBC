...
C**********************************************************************C
C
      SUBROUTINE CALQQ2TGVC (ISTL)
C
C **  THIS SUBROUTINE IS PART OF  EFDC-FULL VERSION 1.0a 
C
C **  LAST MODIFIED BY JOHN HAMRICK ON 1 NOVEMBER 2001
C
C **  08/01/2019        DRBC LZ - COMMENT OUT WAVE DISPERSION CONTRIBUTION WHEN WAVE IS NOT ACTIVATED
C
C----------------------------------------------------------------------C
C
C CHANGE RECORD
...
CLZ       DO K=1,KS
CLZ        DO L=LF,LL
CLZ         TVAR1W(L,K)=(WVDTKEM(K)*WVDISP(L,K)
CLZ     &               +WVDTKEP(K)*WVDISP(L,K+1))
CLZ        ENDDO
CLZ       ENDDO
CLZ      ENDDO
C
...
CLZ      LN=LNC(L)
CLZ      LS=LSC(L)
CLZ      PQQ=DELT*(AB(L,K)*GP*HP(L)*DZIG(K)*(B(L,K+1)-B(L,K))
CLZ     &    +AV(L,K)*DZIGSD4(K)*(U(L+1,K+1)-U(L+1,K)+U(L,K+1)-U(L,K))**2
CLZ     &    +AV(L,K)*DZIGSD4(K)*(V(LN,K+1)-V(LN,K)+V(L,K+1)-V(L,K))**2
CLZ     &    +WVFACT*TVAR1W(L,K) )
CLZ      UUU(L,K)=QQ1(L,K)*H2P(L)
CLZ     &      +DELT*(FUHU(L,K)-FUHU(L+1,K)+FVHU(L,K)-FVHU(LN,K)
CLZ     &      +(FWQQ(L,K)-FWQQ(L,K+1))*DZIG(K))*DXYIP(L)+2.*PQQ
CLZ      VVV(L,K)=QQL1(L,K)*H2P(L)
CLZ     &      +DELT*(FUHV(L,K)-FUHV(L+1,K)+FVHV(L,K)-FVHV(LN,K)
CLZ     &      +(FWQQL(L,K)-FWQQL(L,K+1))*DZIG(K))*DXYIP(L)
CLZ     &      +CTE1*DML(L,K)*PQQ
CLZ      ENDDO
CLZ      ENDDO
C
C     ENDIF
