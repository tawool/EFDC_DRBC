...
!
	  ...
	  IF(TIME.LT.0.0) TIME=ABS(TIME)
        ...
        IABKB=NINT(ABKB)
        IF(NSHTYPE.EQ.2)THEN
          WRITE(6,7)N,ICSHOW,JCSHOW,IZSURF,IVELEKC,IVELNKC,ISALKC,IAVKS,IABKS,IVELEKB,IVELNKB,ISALKB,IAVKB,IABKB
         ELSE
          IF(NSHTYPE.EQ.3)THEN
          WRITE(6,77)TIME,ICSHOW,JCSHOW,IZSURF,IVELEKC,IVELNKC,ISALKC,IAVKS,IABKS,IVELEKB,IVELNKB,ISALKB,IAVKB,IABKB
          ENDIF
          IF(NSHTYPE.GE.5)THEN
...
!     &                            IVELEKB,IVELNKB,ISNDKB,IAVKB,IABKB
          WRITE(6,77)TIME,ICSHOW,JCSHOW,IZSURF,IVELEKC,IVELNKC,ISEDKC,IAVKS,IABKS,IVELEKB,IVELNKB,ISEDKB,IAVKB,IABKB
          WRITE(6,79)ISNDKC,ISNDKB
          ENDIF
          IF(NSHTYPE.EQ.4)THEN
          WRITE(6,77)TIME,ICSHOW,JCSHOW,IZSURF,IVELEKC,IVELNKC,ITEMKC,IAVKS,IABKS,IVELEKB,IVELNKB,ITEMKB,IAVKB,IABKB
          ENDIF
        ENDIF
      ENDIF
!
...
!
    1 FORMAT (80X)
    2 FORMAT('----------------------------------------------------','-------------------------------------------')
    3 FORMAT('| TIME |  I  |  J  | ZSUR | VELE | VELN | SAL |  AV ',1X,'|  AB  | VELE | VELN | SAL |  AV  |  AB  |')
   33 FORMAT('| TIME |  I  |  J  | ZSUR | VELE | VELN | SED |  AV ',1X,'|  AB  | VELE | VELN | SED |  AV  |  AB  |')
   34 FORMAT('| TIME |  I  |  J  | ZSUR | VELE | VELN | TEM |  AV ',1X,'|  AB  | VELE | VELN | TEM |  AV  |  AB  |')
    4 FORMAT('| STEP |     |     |      | SURF | SURF | SUR | SURF',1X,'| SURF | BOTT | BOTT | BOT | BOTT | BOTT |')
    5 FORMAT('|      |     |     |      |      |      |     |     ',1X,'|      |      |      |     |      |      |')
    6 FORMAT('|      |     |     |  CM  | CM/S | CM/S | PSU',1X,'|CMSQ/S|CMSQ/S| CM/S | CM/S | PSU |CMSQ/S|CMSQ/S|')
    7 FORMAT('|',I6   ,'|',I4,1X,'|',I4,1X,'|',I5,1X,'|',I5,1X,'|',I5,1X,'|',I4,1X,'|',I5,1X,'|',I5,1X,'|',I5,1X,'|',I5,1X,'|',I4,1X,'|',I5,1X,'|',I5,1X,'|')
   ...
   44 FORMAT('|      |     |     |      | SURF | SURF | SUR | SURF',1X,'| SURF | BOTT | BOTT | BOT | BOTT | BOTT |')
   66 FORMAT('| DAYS |     |     |  CM  | CM/S | CM/S | PSU',1X,'|CMSQ/S|CMSQ/S| CM/S | CM/S | PSU |CMSQ/S|CMSQ/S|')
   67 FORMAT('|      |     |     |  CM  | CM/S | CM/S |MG/L',1X,'|CMSQ/S|CMSQ/S| CM/S | CM/S |MG/L |CMSQ/S|CMSQ/S|')
   68 FORMAT('| DAYS |     |     |  CM  | CM/S | CM/S | D:C',1X,'|CMSQ/S|CMSQ/S| CM/S | CM/S | D:C |CMSQ/S|CMSQ/S|')
   77 FORMAT('|',F6.1 ,'|',I4,1X,'|',I4,1X,'|',I5,1X,'|',I5,1X,'|',I5,1X,'|',I4,1X,'|',I5,1X,'|',I5,1X,'|',I5,1X,'|',I5,1X,'|',I4,1X,'|',I5,1X,'|',I5,1X,'|')
   79 FORMAT('|',6X ,'|',4X,1X,'|',4X,1X,'|',5X,1X,'|',5X,1X,'|',5X,1X,'|',I4,1X,'|',5X,1X,'|',5X,1X,'|',5X,1X,'|',5X,1X,'|',I4,1X,'|',5X,1X,'|',5X,1X,'|')
!
!**********************************************************************C
!
      RETURN
