...
!
      SUBROUTINE CALIMP2T
!
! **  SUBROUTINE CALEXP CALCULATES IMPLICIT MOMENTUM EQUATION 
! **  CORIOLIS AND CURVATURE TERMS FOR 1/2 STEP PREDICTOR
!
...
!**********************************************************************C
!
! **  CALCULATE TIME LEVEL TIME LEVEL N INTERNAL STRESSES
! **  AND WAVE INDUCED CURRENT FORCINGS
! **  CALCULATE FRACTIONAL STEP WITH STRESSES AND WV FORCES
!
!----------------------------------------------------------------------C
!
      DO ND=1,NDM
...
!       TMPVAL=DZIG(K)
!       DO L=2,LA
!        TVAR2E(L,K)=DZIG(K)*(U(L,K+1)-U(L,K))/AVUI(L,K)
!        TVAR2N(L,K)=DZIG(K)*(V(L,K+1)-V(L,K))/AVVI(L,K)
!       ENDDO
!      ENDDO
!      TVAR2E(L,KC)=TSX(L)
!      TVAR2N(L,KC)=TSY(L)
...
!      DO K=2,KC
!       DO L=2,LA
!        TVAR2W(L,K)=DZIG(K-1)*(U(L,K)-U(L,K-1))/AVUI(L,K-1)
!        TVAR2S(L,K)=DZIG(K-1)*(V(L,K)-V(L,K-1))/AVVI(L,K-1)
!       ENDDO
!      ENDDO
!
...
!       TMPVAL=DELTD2*DZIC(K)
!       DO L=2,LA
!        TVAR1W(L,K)=DYIU(L)*UHDY(L,K)+DELTD2*DZIC(K)*(TVAR2E(L,K)-TVAR2W(L,K))
!        TVAR1S(L,K)=DXIV(L)*VHDX(L,K)+DELTD2*DZIC(K)*(TVAR2N(L,K)-TVAR2S(L,K))
!       ENDDO
!      ENDDO
!
      IF(ISWAVE.GE.2)THEN
...
!
      DO K=1,KC
       DO L=2,LA
        LS=LSC(L)
        TVAR1W(L,K)=TVAR1W(L,K)-TVAR3W(L)*(+H1U(L)*(TVAR1E(L,K)-TVAR1E(L-1,K))+0.5*GP*(B(L,K)     &
                    +B(L-1,K))*(BELV(L)-BELV(L-1))+0.5*(TVAR1N(L,K)+TVAR1N(L-1,K))*(HP(L)-HP(L-1)))              
        TVAR1S(L,K)=TVAR1S(L,K)-TVAR3S(L)*(+H1V(L)*(TVAR1E(L,K)-TVAR1E(LS ,K))+0.5*GP*(B(L,K)     &
                    +B(LS,K))*(BELV(L)-BELV(LS))+0.5*(TVAR1N(L,K)+TVAR1N(LS ,K))*(HP(L)-HP(LS)))              
       ENDDO
      ENDDO
!
!**********************************************************************C
!
! **  ADJUST INTERNAL TRANSPORT AT OPEN BOUNDARIES
!
!----------------------------------------------------------------------C
!
! **  EXPLICIT PART OF HALF STEP IS COMPLETE WITH 
! **  TVAR1W = (UH)*, TVAR1S = (VH)*
!
!**********************************************************************C
!
...
!**********************************************************************C
!
! **  ADVECTION STEP FOR X MOMENTUM EQUATION
...
!**********************************************************************C
!
! **  ADVECTIVE FLUX HALF STEP
! **  CALCULATE ADVECTIVE FLUXES BY UPWIND DIFFERENCE WITH ADVECTION
! **  AT N AND ADVECTED FIELD AT N 
!
!----------------------------------------------------------------------C
!
! **  UHDY2(L,K) IS DYU*U ON POS X U(L) MOM CV FACE (AT B OR P POINT)
! **  VHDX2(L,K) IS DXV*V ON NEG Y U(L) MOM CV FACE (AT CORNER POINT)
! **  W2(L,K)    IS DXYP*W/H ON TOP U(L) MOM CV FACE
!
! ** SMOLARKIEZC AND MARGOLIN FORM
!
!      DO K=1,KC
!       DO L=2,LA
!        UHDY2(L,K)=0.25*STCUV(L)*( DYU(L  )*(U(L  ,K)+U(L  ,K))
!     &                              +DYU(L+1)*(U(L+1,K)+U(L+1,K)) )
!        VHDX2(L,K)=0.25*( DXV(L  )*(V(L  ,K)+V(L  ,K))
!     &                   +DXV(L-1)*(V(L-1,K)+V(L-1,K)) )
!       ENDDO
!      ENDDO
!
!      DO K=1,KS
!       DO L=2,LA
!        W2(L,K)=0.25*( DXYP(L  )*(W(L  ,K)/HP(L  )+W(L  ,K)/HP(L  ))
!     &                +DXYP(L-1)*(W(L-1,K)/HP(L-1)+W(L-1,K)/HP(L-1)) )
!       ENDDO
!      ENDDO
!
! ** STANDARD FORM
!
      DO K=1,KC
       DO L=2,LA
        UHDY2(L,K)=0.25*STCUV(L)*((UHDY(L,K)+UHDY(L,K))+(UHDY(L+1,K)+UHDY(L+1,K)))
        VHDX2(L,K)=0.25*((VHDX(L,K)+VHDX(L,K))+(VHDX(L-1,K)+VHDX(L-1,K)) )
       ENDDO
      ENDDO
!
      DO K=1,KS
       DO L=2,LA
        W2(L,K)=0.25*( DXYP(L)*(W(L,K)+W(L,K))+DXYP(L-1)*(W(L-1,K)+W(L-1,K)) )
       ENDDO
      ENDDO
!
!----------------------------------------------------------------------C
...
!
      DO K=1,KC
       DO L=2,LA
        FUHU(L,K)=MAX(UHDY2(L,K),0.)*U(L,K)+MIN(UHDY2(L,K),0.)*U(L+1,K)
        FVHU(L,K)=MAX(VHDX2(L,K),0.)*U(LSC(L),K)+MIN(VHDX2(L,K),0.)*U(L,K)
       ENDDO
      ENDDO
!
      DO K=1,KS
       DO L=2,LA
        FWU(L,K)=MAX(W2(L,K),0.)*U(L,K)+MIN(W2(L,K),0.)*U(L,K+1)
       ENDDO
      ENDDO
!
...
!**********************************************************************C
!
! **  ADVANCE U ADVECTION 1/2 TIME STEP
!
!----------------------------------------------------------------------C
!
      DO K=1,KC
...
!**********************************************************************C
!
! **  ADVECTION STEP FOR Y MOMENTUM EQUATION
! **  SELECT ADVECTIVE FLUX FORM
!
!----------------------------------------------------------------------C
!
...
!----------------------------------------------------------------------C
!
! **  UHDY2(L,K) IS DYU*U ON NEG X V(L) MOM CV FACE (CORNER POINT)
! **  VHDX2(L,K) IS DXV*V ON POS Y V(L) MOM CV FACE (B 0R P POINT)
! **  W2(L,K)    IS DXYP*W/H ON TOP V MOM CV FACE
!
! ** SMOLARKIEWCZ AND MARGOLIN FORM
!
!      DO K=1,KC
!       DO L=2,LA
!        LS=LSC(L)
!        LN=LNC(L)
!        UHDY2(L,K)=0.25*( DYU(L )*(U(L ,K)+U(L ,K))
!     &                   +DYU(LS)*(U(LS,K)+U(LS,K)) )
!        VHDX2(L,K)=0.25*STCUV(L)*( DXV(L )*(V(L ,K)+V(L ,K))
!     &                            +DXV(LN)*(V(LN,K)+V(LN,K)) )
!       ENDDO
!      ENDDO
!
!      DO K=1,KS
...
!        LS=LSC(L)
!        W2(L,K)=0.25*( DXYP(L )*(W(L ,K)/HP(L )+W(L ,K)/HP(L ))
!     &                +DXYP(LS)*(W(LS,K)/HP(LS)+W(LS,K)/HP(LS)) )
!       ENDDO
!      ENDDO
!
! ** STANDARD FORM
!
      DO K=1,KC
       DO L=2,LA
        LS=LSC(L)
        LN=LNC(L)
        UHDY2(L,K)=0.25*((UHDY(L,K)+UHDY(L,K))+(UHDY(LS,K)+UHDY(LS,K)))
        VHDX2(L,K)=0.25*STCUV(L)*((VHDX(L,K)+VHDX(L,K))+(VHDX(LN,K)+VHDX(LN,K)))
       ENDDO
      ENDDO
!
      DO K=1,KS
       DO L=2,LA
        LS=LSC(L)
        W2(L,K)=0.25*(DXYP(L)*(W(L,K)+W(L,K))+DXYP(LS)*(W(LS,K)+W(LS,K)) )
       ENDDO
      ENDDO
!
!----------------------------------------------------------------------C
...
!
      DO K=1,KC
       DO L=2,LA
        FUHV(L,K)=MAX(UHDY2(L,K),0.)*V(L-1,K)+MIN(UHDY2(L,K),0.)*V(L,K)
        FVHV(L,K)=MAX(VHDX2(L,K),0.)*V(L,K)+MIN(VHDX2(L,K),0.)*V(LNC(L),K)
       ENDDO
      ENDDO
!
      DO K=1,KS
       DO L=2,LA
        FWV(L,K)=MAX(W2(L,K),0.)*V(L,K)+MIN(W2(L,K),0.)*V(L,K+1)
       ENDDO
      ENDDO
!
!**********************************************************************C
...
!**********************************************************************C
!
! **  ADVANCE V ADVECTION 1/2 TIME STEP
!
!----------------------------------------------------------------------C
!
      DO K=1,KC
...
!**********************************************************************C
!
! **  ADVANCE CONTINUITY EQUATION 1/2 STEP
!
!
!**********************************************************************C
!
! **  INITIALIZE MID TIME LEVEL VELOCITY
!
!
!
!**********************************************************************C
!
! **  BEGIN CORIOLIS AND CURVATURE ACCELERATION ITERATION
!
!**********************************************************************C
!
! **  CALCULATE CORIOLIS AND CURVATURE PARAMETER
!
!----------------------------------------------------------------------C
!
      DO K=1,KC
...
!
      DO K=1,KC
       ...
       LNW=LNWC(L)
       LSE=LSEC(L)
       FCAX(L,K)=0.25*SCAX(L)*(CAC(L,K)*(V(LN,K)+V(L,K))+CAC(L-1,K)*(V(LNW,K)+V(L-1,K)))
       FCAY(L,K)=0.25*SCAY(L)*(CAC(L,K)*(U(L+1,K)+U(L,K))+CAC(LS,K)*(U(LSE,K)+U(LS,K)))
       ENDDO
      ENDDO
!
!**********************************************************************C
!
! **  CALCULATE MID TIME LEVEL TRANSPORT AND VELOCTIY
!
!----------------------------------------------------------------------C
!
       DO K=1,KC
...
!**********************************************************************C
!
! **  ADD REMAINING SURFACE AND BOTTOM STRESS
!
!----------------------------------------------------------------------C
! 
      DO L=2,LA
...
!
      ...
      DO L=2,LA
      LS=LSC(L)
      FBBX(L,K)=SBX(L)*GP*HU(L)*(HU(L)*((B(L,K+1)-B(L-1,K+1))*DZC(K+1)+(B(L,K)-B(L-1,K))*DZC(K))  &
                -(B(L,K+1)-B(L,K)+B(L-1,K+1)-B(L-1,K))*(BELV(L)-BELV(L-1)+Z(K)*(HP(L)-HP(L-1))))
      FBBY(L,K)=SBY(L)*GP*HV(L)*(HV(L)*((B(L,K+1)-B(LS,K+1))*DZC(K+1)+(B(L,K)-B(LS,K))*DZC(K))    &
...
!
      DO K=1,KS
       RCDZF=CDZF(K)
       DO L=2,LA
        DU(L,K)=RCDZF*( 2.*(TVAR1W(L,K+1)-TVAR1W(L,K))*DELTI+DXYIU(L)*FBBX(L,K) )
        DV(L,K)=RCDZF*( 2.*(TVAR1S(L,K+1)-TVAR1S(L,K))*DELTI+DXYIV(L)*FBBY(L,K) )
       ENDDO
      ENDDO
!
      DO ND=1,NDM
