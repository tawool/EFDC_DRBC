...
C
      ...
      DIMENSION DTL1(LCM),DTL2(LCM),DTL3(LCM)
C
C**********************************************************************C
C
      ITRNTMP=0
...
C
c      RTCTMP=FLOAT(N)/FLOAT(NTSPTC)
c      NTCTMP=RTCTMP
c      NTMP=(1+NTCTMP)*NTSPTC-N
c      IF(NINCRMT.GT.NTMP)THEN
c        NINCRMT=NTMP
c        DTDYN=FLOAT(NTMP)*DTMIN
c      ENDIF
C
